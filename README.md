...
(диалоговый агент, исполнитель, маршрутизатор) и медиа-вызовы работают на моделях OpenCode Go
($10/мес, `https://opencode.ai/zen/go/v1`). По умолчанию везде Muse Spark 1.3 Contributor, в настройках
каждая роль переключается на любую другую модель подписки (Grok, GPT, GLM, Kimi, DeepSeek, Qwen и др.),
а для Muse Spark на каждую роль задаётся свой уровень reasoning (minimal/low/medium/high/xhigh).
Интерфейс, распознавание речи (faster-whisper), синтез речи (TeraTTSv2), инструменты и память —
без изменений. Долговременная память по-прежнему считается эмбеддингами NVIDIA NIM.
...
- **Сервер** (`server/`, Python, FastAPI). Принимает соединения клиентов по WebSocket, ведёт контекст
  разговора каждой сессии, вызывает модель Muse Spark 1.3 Contributor через OpenCode Go (Responses API)
...
flowchart LR
    ...
    subgraph GO["OpenCode Go (opencode.ai/zen/go/v1)"]
        direction TB
        M1["Muse Spark 1.3 Contributor и др.<br/>(текст: responses- и chat-транспорты)"]
        M2["Мультимодальные (медиа)"]
    end
    ...
    SS -- "responses (Muse, Grok, GPT, DeepSeek)" --> M1
    SS -- "chat/completions (GLM, Kimi, Qwen и др.)" --> M1
    SS -- "медиа как input_image" --> M2
    SS -- "embeddings" --> M3
```
...
Таблица ниже — базовые замеры NemoAgent (NVIDIA NIM); для SparkAgent (OpenCode Go, Responses API)
...
- `OPENCODE_API_KEY`: ключ провайдера OpenCode Go (`/connect` в приложении OpenCode → хранится в
  `~/.local/share/opencode/auth.json` под именем `opencode-go`); `GO_BASE_URL` (по умолчанию
  `https://opencode.ai/zen/go/v1`), `GO_MODEL` (по умолчанию `muse-spark-1.3-contributor`);
...
- `LLM_PROXY`: необязательный прокси (http/https/socks5) только для Go API — нужен, если прямой маршрут
  до `opencode.ai` недоступен;
...
Настройки (правая колонка): модель на каждую роль (по умолчанию везде Muse Spark 1.3 Contributor;
на выбор — все модели подписки OpenCode Go: Grok 4.6, GPT 5.6 Luna, GLM 5.x, Kimi K3/K2.7/K2.6, LongCat 2.0,
DeepSeek V4.x, MiMo V2.5, MiniMax M3/M2.7/M2.5, Qwen3.8/3.7/3.6, Hy4/Hy3, Union Alpha; медиа-роль —
мультимодальные: оба Muse Spark и DeepSeek Vision Exp) и уровень размышлений на каждую роль
(шкала своя у каждой модели — селект перестраивается под выбранную; дефолты: агент minimal,
остальные xhigh), режим озвучки, язык озвучки («авто» по тексту
...
- `nim.py`: `NIMClient` (имя сохранено для совместимости): текстовые вызовы через OpenCode Go.
  Два транспорта по модели: Responses (`instructions` + `input`, function tools; Muse, Grok, GPT, DeepSeek) —
  диалог/маршрутизатор стримят дельты, исполнитель — один нестриминговый запрос на раунд; Chat
  (`/chat/completions`, OpenAI SSE; GLM, Kimi, Qwen и др.) — всегда стрим, `<think>`-блоки вырезаются
  из речи в события reasoning. Fingerprint по докам Go (свой UA `SparkAgent/1.0`, стабильный
  `x-opencode-session` на диалог, свежий `x-opencode-request`).
  Reasoning effort по ролям со шкалой каждой модели (дефолты: агент minimal, остальные xhigh;
  responses: `reasoning.effort`, chat: `reasoning_effort`; неподдержанное значение опускается), `temperature` не отправляется (гейт пинит его сам), `tool_choice`
  только `auto` (`required` эмулируется директивой), повтор при перегрузке пула,
  детектор зацикливания генерации; эмбеддинги — по-прежнему в NIM.
...
- OpenCode Go — подписка $10/мес с лимитами ($60/мес на Muse Spark 1.3 Contributor, ~$0.10/$0.20 за 1M
  токенов). Лимиты: 5 часов — 20%, неделя — 50%. Использование видно в консоли `opencode.ai/auth`.
  `LLM_THINKING` оставлен для совместимости и ни на что не влияет.
- Бесплатный пул общий и ограниченный: в часы пик шлюз отвечает 429/503/529.
  Сервер повторяет запрос автоматически. Отдельно: бесплатный `-free` тир Zen отдаётся только внутри
  приложения OpenCode — SparkAgent использует платный Go endpoint, его это не касается.
- Учитывайте бюджет reasoning: thinking-токены входят в `max_output_tokens`. При исчерпании ответ
  возвращается обрезанным с пометкой (`length`), а не ошибкой.
- `minimax-m2.7` и `union-alpha` периодически отдают 500 на стороне шлюза — сервер повторяет запрос,
  но при затяжном простое переключите роль на соседнюю модель.

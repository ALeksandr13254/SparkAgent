...
(minimal/low/medium/high/xhigh — действует только на Muse Spark, остальные модели игнорируют), режим озвучки, язык озвучки («авто» по тексту
...
- `nim.py`: `NIMClient` (имя сохранено для совместимости): текстовые вызовы через OpenCode Go.
  ...
  Reasoning effort по ролям (диалог `low`, маршрутизатор `minimal`, исполнитель `medium`; гейт по умолчанию
  ставит `high` и съедает бюджет), `temperature` не отправляется (гейт пинит его сам), `tool_choice`

...
Muse Spark 1.3 Contributor через OpenCode Go (Responses API). Интерфейс, распознавание речи
...
flowchart LR
    ...
    subgraph GO["OpenCode Go (opencode.ai/zen/go/v1, Responses API)"]
        direction TB
...
- `nim.py`: `NIMClient` (имя сохранено для совместимости): текстовые вызовы через OpenCode Go Responses API
  (`instructions` + `input`, `max_output_tokens`, function tools; диалог/маршрутизатор — стрим дельт
  `response.output_text.delta`, исполнитель — один нестриминговый запрос на раунд), fingerprint по докам Go
  (свой UA `SparkAgent/1.0`, стабильный `x-opencode-session` на диалог, свежий `x-opencode-request`),
